...
const [suggestion, setSuggestion] = useState({ text: "", actionType: "meditation" });
  ...
  const handleDevSparkClick = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_LOCAL_API }/api/sparks/complete`, {
        method: 'POST',
        headers: { 
...
useEffect(() => {
    const fetchDashboardTip = async () => {
      try {
        ...
        const resData = await axios.get(`${import.meta.env.VITE_LOCAL_API }/api/insights/weekly`, {
        ...
        const tipRes = await axios.post(`${import.meta.env.VITE_LOCAL_API }/api/insights/dashboard-tip`, {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const res = await fetch(`${import.meta.env.VITE_LOCAL_API }/api/users/profile`, {
    ...
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const [userRes, journalRes, moodRes] = await Promise.all([
          fetch(`${import.meta.env.VITE_LOCAL_API }/api/users/profile`, { headers: { 'Authorization': `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_LOCAL_API }/api/journals`, { headers: { 'Authorization': `Bearer ${token}` } }),
          fetch(`${import.meta.env.VITE_LOCAL_API }/api/moods`, { headers: { 'Authorization': `Bearer ${token}` } })
        ]);
